				...
				if (rs.status == 1) {
					...
					setTimeout(function () {
						...
						if ($.trim(mem.mem_city).length != 0 && $.trim(mem.mem_address).length != 0 && $.trim(mem.mem_zip).length != 0) {
							select_address += `<option data-type="home" value="${mem.mem_address} @@ ${mem.mem_city} @@ ${mem.mem_zip}" data-lat="${mem.mem_map_lat}" data-long="${mem.mem_map_lng}" data-full-address="Home: <br/> ${mem.mem_address} <br/> ${mem.mem_city} <br/> ${mem.mem_zip}">
						...
						if ($.trim(mem.mem_business_city).length != 0 && $.trim(mem.mem_business_address).length != 0 && $.trim(mem.mem_business_zip).length != 0) {
							select_address += ` <option data-type="office" value="${mem.mem_business_address} @@ ${mem.mem_business_city} @@ ${mem.mem_business_zip}" data-lat="${mem.mem_business_map_lat}" data-long="${mem.mem_business_map_lng}" data-full-address="Office: <br/> ${mem.mem_business_address} <br/> ${mem.mem_business_city} <br/> ${mem.mem_business_zip}">
						...
						if ($.trim(mem.mem_hotel_city).length != 0 && $.trim(mem.mem_hotel_address).length != 0 && $.trim(mem.mem_hotel_zip).length != 0) {
							select_address += `<option data-type="hotel" value="${mem.mem_hotel_address} @@ ${mem.mem_hotel_city} @@ ${mem.mem_hotel_zip}" data-lat="${mem.mem_hotel_map_lat}" data-long="${mem.mem_hotel_map_lng}" data-full-address="Hotel: <br/> ${mem.mem_hotel_address} <br/> ${mem.mem_hotel_city} <br/> ${mem.mem_hotel_zip}">
						...
						select_address += `</div>`;
						$("#select-address-after-login").removeClass("hidden").html(select_address);

						// let previous_button_icon = '<?php base_url("") ?>';
						$("#continue-buttons").append(`<button type="button" class="site_btn md light prev_btn"><img src="${base_url}assets/images/arrow-left-sm.svg" alt=""> Previous step</button>
				...
				if (rs.status == 1) {
					setTimeout(function () {
						...
						$("#deliveryProofRequest")
							...
							.append(`<div class="alert alert-success alert-sm text-white" style="">Request has been accepted successfully.</div>`);
				...
				if (rs.status == 1) {
					setTimeout(function () {
						...
						$("#deliveryProofRequest")
							...
							.append(`<div class="alert alert-success alert-sm text-white" style="">Request has been rejected successfully.</div>`);
...
const fetchTimeByInterval = (day, mem_id, appendTo) => {
	$.ajax({
		url: base_url + "index/fetch_time_by_interval",
