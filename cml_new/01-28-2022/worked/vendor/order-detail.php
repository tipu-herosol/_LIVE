...
<body data-page="dash">
    ...
    <main>
        ...
        <section id="orders">
                        ...
                        <div class="txt">
                            <div class="top_head">
                                ...
                                <?php if ($order->order_status == 'New') : ?>
                                    <div class="status_btn_blk" id="order-status-dropdown">
                                        <?php echo order_status_dropdown($order->order_status, $order->order_id, 'vendor') ?>
                                    </div>
                                <?php else : ?>
                            ...
                            <ul class="list">
                                ...
                                <li>
                                    ...
                                    <span id="customer_notes_blk"><?= $order->order_note ?></span>
                                    <!-- <div class="form_blk" id="customer_notes_blk">
                                        <textarea name="customer_notes" id="customer_notes" class="text_box" placeholder="Type Notes" <?= ($order->order_status != 'Cancelled' && $order->order_status != 'Completed' && $order->order_status != 'Delivered') ? '' : 'disabled' ?> readonly><?= $order->order_note ?></textarea>
                                        <?php if (($order->order_status != 'Cancelled' && $order->order_status != 'Completed' && $order->order_status != 'Delivered')) :  ?>
                                            <button type="button" id="save-notes" class="site_btn blank edit_notes" data-o-id="<?= doEncode($order->order_id) ?>"></button>
                                        <?php endif;  ?>
                                    </div> -->
                                </li>
                            </ul>
                            <?php if ($order->order_status != 'Cancelled' && $order->order_status != 'Completed' && $order->order_status != 'Delivered') : ?>
                                <div class="btn_blk form_btn text-right">
                                    <button class="site_btn text pop_btn" data-popup="notes"><u>Leave note to customer</u></button>
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
                                    ...
                                    <div class="txt">
                                        <div class="ico_txt">
                                            <div class="title">
                                                ...
                                                <div class="rating bg">
                                                    <div class="rateYo" data-rateyo-rating="<?= $review->rating ?>" data-rateyo-star-width="20px"></div>
                                        ...
                                        <div class="date text-right"><?= date_picker_format_date($review->review_date, 'D, d M Y', false) ?></div>
                                    </div>
                                </div>
                            </div>
                        <?php endif; ?>
                                ...
                                <form action="<?= base_url() ?>vendor/complete_order" method="post" class="frmCompleteOrder">
                                    ...
                                    <div class="form_row row">
                                        <div class="col-xs-12">
                                            ...
                                            <div class="form_blk">
                                                <div class="img_proof_row flex">
                                                    <div class="img">
                                                        <figure>
                                                            <button type="button" class="x_btn"></button>
                                                        </figure>
                                                        <button type="button" class="plus uploadImg" data-upload="bag_pic"></button>
                                                    </div>
                                                    <div class="img">
                                                        <figure>
                                                            <button type="button" class="x_btn"></button>
                                                ...
                                                <input type="file" name="proof_image[]" id="proof_image" class="uploadFile" data-upload="bag_pic" multiple>
                                                <span class="validation-error" id="proof_image_error"></span>
                                            </div>
                                        </div>
                                        <div class="col-xs-12">
                                            <h6>Comment on order</h6>
                                            <div class="form_blk">
            ...
            <?php if ($order->order_status != 'Cancelled' && $order->order_status != 'Completed' && $order->order_status != 'Delivered') : ?>
                <div class="popup sm" data-popup="notes">
                    <div class="table_dv">
                        <div class="table_cell">
                            <div class="contain">
                                <div class="_inner">
                                    <div class="cross_btn"></div>
                                    <h4>Order Note to Customer</h4>
                                    <div class="form_blk">
                                        <textarea name="customer_notes" id="customer_notes" class="text_box" placeholder="Type anything here"><?= $order->order_note ?></textarea>
                                    </div>
                                    <div class="btn_blk text-center">
                                        <button type="button" id="save-notes" class="site_btn long cross_btn" data-o-id="<?= doEncode($order->order_id) ?>">Submit</button>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <script>
            $(function() {
                ...
                var imagesPreview = function(input, placeToInsertImagePreview) {
                    ...
                    $('#proof_image_error').html('');
                    if (input.files) {
                        var filesAmount = input.files.length;
                        if (filesAmount > 3) {
                            $('#proof_image').val('');
                            $('#proof_image_error').html('Please select maximum 3 images.');
                            return false;
                        }
                        ...
                        for (i = 0; i < filesAmount; i++) {
                            ...
                            reader.onload = function(event) {
                                html = `<div class="img">
                                            <figure>
                                                <img src="${event.target.result}" alt="" data-src="" id="">
                                                <button type="button" class="x_btn"></button>
                                            </figure>
                                        </div>`;
                ...
                $('#proof_image').on('change', function() {
                    imagesPreview(this, 'div.img_proof_row');
                });
            });
            ...
            $(function() {
                $(document).on('click', '#save-notes', function(e) {
                    ...
                    $("#customer_notes_blk, #customer_notes").html(notes);
                    if (notes.length !== 0) {} else {}
                    $.ajax({
            ...
            $(function() {
                $(document).on('click', '#add-amendment-row', function(e) {
                    ...
                    let html = `<div class="col-xs-12" id="row-${rowIndex}">
                                    ...
                                    <div class="btn_blk mt text-right">
                                        <button type="button" class="site_btn text danger" onclick="deleteThisRow(${rowIndex})">Delete</button>
                                    </div>
                                </div>`;
