    ...
    <main>
        ...
        <section id="quotes">
            <div class="contain">
                <div class="flex_row">
                    <div class="col col1">
                        <div id="filter_blk" class="blk filters">
                            <div class="scrollbar">
                                <p id="showing_records"></p>
                                ...
                                <form id="searchForm">
                                    ...
                                    <div class="in_blk">
                                        <h6>Distance <small>(10 miles)</small></h6>
        ...
        <script type="text/javascript">
            ...
            var map, bounds, startLat = locations[0][1],
            ...
            $(document).on("click", "#map_btn", function() {
                $("#locate_blk").addClass("active");
            });
            $(document).on("click", "#locate_blk .cross_btn", function() {
                $("#locate_blk").removeClass("active");
            });
            
            $(document).on('click', '#clear', function(e) {
                e.preventDefault();
                var price_filter = $('#price').data('ionRangeSlider');
                var distance_filter = $('#distance').data('ionRangeSlider');
                price_filter.reset();
            ...
            function search(btn = null) {
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    beforeSend: function() {
                        $('#quotes-section').html('<h4>Fetching quotes...</h4>');
                    },
                    success: function(res) {
                        setTimeout(() => {
                            if (btn !== null) {
                                frmIcon.addClass('hidden');
                                frmIcon.removeAttr('disabled');
                            }
            ...
            function setMarkers(map, locations) {
                ...
                if (locations.length > 0) {
                    for (var i = 0; i < locations.length; i++) {
                        ...
                        google.maps.event.addListener(markers[i], 'mouseover', (function(inneri) {
                            return function() {
                                highlightQuote(inneri);
                            }
                        })(i));
                        google.maps.event.addListener(markers[i], 'mouseout', (function(inneri) {
                            return function() {
                                unHighlightQuote(inneri);
                            }
                        })(i));
            ...
            {
                $('.srch_blk:eq('+ index +') > a').css({'border-color': '#008cff'});
                
            }

            function unHighlightQuote(index)
            {
                $('.srch_blk:eq('+ index +') > a').css({'border-color': ''});
            ...
            function highlightedIcon() {
                return {
                    url: base_url + "assets/images/highlighted_marker.svg", // url
                    scaledSize: new google.maps.Size(50, 50), // scaled size
                    origin: new google.maps.Point(0, 0), // origin
                    anchor: new google.maps.Point(25, 50) // anchor
                };
            ...
            // $(document).ready(function() {
            // make a .hover event
            $(document).on('mouseover', '.quotes .srch_blk', function(){
                var index = $('.quotes .srch_blk').index(this);
                markers[index].setIcon(highlightedIcon());
            });

            $(document).on('mouseout', '.quotes .srch_blk', function(){
                var index = $('.quotes .srch_blk').index(this);
