...
function get_promo_code_used_count($id)
{
    global $CI;
    $CI->db->where(['promo_code_id' => $id]);
    $row = $CI->db->get('promo_code_records');
    return intval($row->num_rows());
}
...
function get_mem_com_name($mem_id)
{
    global $CI;
    $CI = get_instance();
    $row = $CI->master->getRow('members', array('mem_id' => $mem_id));
    return ucwords($row->mem_company_name);
}
...

{
    ...
    if (empty($rows)) {
        ...
        foreach ($rows as $key => $row) :
            ...
            <div class="done_blk"><div class="img_row">';
            for($i = 1; $i < 4; $i++) :
                $proof_image_index = 'proof_image_'.$i;
                if(!empty($row->$proof_image_index)):
                    $html .= '<figure><a href="' . get_site_image_src("orders", $row->$proof_image_index, '') . '" class="image" data-fancybox="proof">
                            <img src="' . get_site_image_src("orders", $row->$proof_image_index, '400p_') . '" alt="" data-src="' . get_site_image_src("orders", $row->$proof_image_index, '') . '">
                        </a></figure>';
                endif;
            endfor;
            $html .= '</div>
                    <div class="txt">
                        <h6>Comments</h6>
                        <span class="badge hidden ' . get_delivery_proof_status($row->status) . '">' . ucfirst($row->status) . '</span>
                        <p>' . $row->proof_comment . '</p>
                </div>';
            if($row->status == 'rejected')
            {
                $html .= '<br/><div class="txt">
                        <h6>Rejection Reason</h6>
                        <p>' . $row->proof_reject_reason . '</p>
                </div>';
            }
...
function amended_invoice($order_id, $amended_records)
{
    ...
    if ($CI->session->mem_type == 'buyer') {
        $payButton .= '<button type="button" class="site_btn xs light pop_btn" data-popup="pay-amend-invoice">Pay</button>';
    }
    ...
    if (!empty($amended_records)) :
        ...
        if ($amend_pending > 0) {
            if ($CI->session->mem_type == 'buyer') {
                $html .= '<tr>
                        <th colspan="4" class="color text-right">Pending <strong id="pending-amend-invoice-amount">£' . price_format($amend_pending) . '</strong></th>
    ...
    $html .= '<tr class="">
                <th class="text-left" colspan="">Reason for qmendment:</th>
                <td class="text-center">' . $row->amendment_reason . '</td>
            </tr>';
...
function order_total_price($order_id, $what_to_return = 'FINAL')
{
    ...
    if($order->buyer_promo_discount > 0)
        $total_price -= $order->buyer_promo_discount;
